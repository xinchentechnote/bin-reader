...
TEST(AppStateTest, ReadLengthPrefixedString) {
    ...
    state.cursor_pos = 0;
    EXPECT_THROW(state.read_length_prefixed_string<uint8_t>(0), std::out_of_range);
...
TEST(AppStateTest, CursorMovementAndPages) {
    ...
    state.hex_view_h = 2;
    // total_lines = ceil(100/10) = 10 行；lines_per_page = max(1,2)=2；total_pages = ceil(10/2)=5
...
TEST(ReaderFactoryTest, FixStringReader_SuccessAndFail) {
    {
        ...
        state.cursor_pos = 0;
        bool ok1 = ReaderFactory::instance().read(state, "string");
        EXPECT_TRUE(ok1);
        EXPECT_EQ(state.cursor_pos, 5u);
    ...
    }
    {
        ...
        state2.data = { 'A', 'B', 'C' };
        state2.cursor_pos = 0;
        bool ok2 = ReaderFactory::instance().read(state2, "string");
        EXPECT_FALSE(ok2);
        // status_msg 至少要包含 "String read failed"
        EXPECT_NE(state2.status_msg.find("String read failed"), std::string::npos);
    }
}
...
TEST(ReaderFactoryTest, LengthPrefixedStringReaders) {
    // 1) string@u8
    {
        ...
        {
            auto& msg = state.status_msg;
            EXPECT_NE(msg.find("Read lpstring:"), std::string::npos);
            EXPECT_NE(msg.find("@ 0x0"), std::string::npos);
        }
    }
    // 2) string@u16
    ...
    // 3) string@i8 越界测试
    {
        ...
        state.cursor_pos = 0;
        EXPECT_THROW(ReaderFactory::instance().read(state, "string@i8"), std::out_of_range);
        // cursor_pos 应恢复到 0
        EXPECT_EQ(state.cursor_pos, 0u);
    }
}
...
TEST(RecordTest, DescriptionFormatting) {
    ...
    r1.type_name = "u16";
    // description: index:0x00000010; type_name:u16; ... 这里只验证前缀部分包含 "00000010:u16:"
